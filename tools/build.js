...
.stat-panel.on{animation:statIn .6s cubic-bezier(.2,.8,.2,1)}
.stat-label{font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--accent)}
/* 슬라이드 위치 점: 클릭하면 그 슬라이드로, 활성 점은 길어지면서 남은 시간을 채워 보여줌 */
.stat-dots{display:flex;gap:8px;margin-top:22px}
.stat-dot{position:relative;width:8px;height:8px;padding:0;border:0;border-radius:999px;background:rgba(255,255,255,.22);cursor:pointer;overflow:hidden;transition:width .3s,background .3s}
.stat-dot:hover{background:rgba(255,255,255,.45)}
.stat-dot[aria-selected="true"]{width:40px;background:rgba(255,255,255,.22)}
.stat-dot[aria-selected="true"]::after{content:"";position:absolute;inset:0;background:var(--fg);transform-origin:left;transform:scaleX(0);animation:statFill var(--stat-ms,5000ms) linear forwards}
.stats.paused .stat-dot[aria-selected="true"]::after{animation-play-state:paused}
.stat-dot:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
@keyframes statFill{to{transform:scaleX(1)}}
...
@keyframes statIn{from{opacity:0;transform:translateX(28px)}to{opacity:1;transform:none}}
@media(prefers-reduced-motion:reduce){.stat-panel.on{animation:none}.stat-dot[aria-selected="true"]::after{animation:none;transform:none}}
...
/* 9. 첫 화면 전광판: 자동으로 넘어가는 슬라이드 (점 클릭 · 방향키 · Home · End 로도 이동) */
const statTabs = [...$('.stat-dot')];
if (statTabs.length) {
  const box = statTabs[0].closest('.stats');
  const panels = statTabs.map(t => document.getElementById(t.getAttribute('aria-controls')));
  const ms = parseInt(getComputedStyle(box).getPropertyValue('--stat-ms')) || 5000;
  ...
  const restart = () => {
    clearTimeout(timer);
    if (!still && !box.classList.contains('paused')) timer = setTimeout(() => pick((cur + 1) % statTabs.length, false), ms);
  };
  // 마우스 올리거나 키보드 포커스가 있거나 탭이 안 보이면 멈춤
  const pause = () => { box.classList.add('paused'); clearTimeout(timer); };
  const resume = () => { box.classList.remove('paused'); restart(); };
  box.addEventListener('mouseenter', pause);
  box.addEventListener('mouseleave', resume);
  box.addEventListener('focusin', pause);
  box.addEventListener('focusout', e => { if (!box.contains(e.relatedTarget)) resume(); });
